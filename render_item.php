...
//include 'site/libraries/hunter.php';
...
$hashQuery = mysqli_query($conn, "SELECT `hash` FROM `items` WHERE `id` = $itemId");
$hash = mysqli_fetch_assoc($hashQuery)['hash'];
...
$_SESSION['SAVE_ITEM_THUMB_TOKEN'] = token();

	$getItemTypeByIdQ = mysqli_query($conn, "SELECT `type` FROM `items` WHERE `id` = $itemId");
	$itemType = (int)mysqli_fetch_assoc($getItemTypeByIdQ)["type"];
...
//echo '<script>//'.var_dump($avatar).'</script>';
...
function init() {
    ...
    loader.load('".($avatar["face"] ? "/storage/items/textures/$hash.png" : 'face.png')."', function(faceimg) {
        facetex.image = faceimg;
        facetex.needsUpdate = true;
    });
	
	loader.load('studs.png', function(studimg) {
...
if(window.jQuery) {
	setTimeout(function() {
		$.ajax({
			url: '/save_item_thumbnail.php',
			method: 'POST',
			data: {
				'token': '{$_SESSION['SAVE_ITEM_THUMB_TOKEN']}',
				'hash': '$hash',
			...
			success: function(){
				$.ajax({
					...
					method:'POST',
					success:function(z){
						window.location='/meta_item_img.php?item_id=$itemId';
						window.top.location = '/item.php?id=$itemId';
						}
				});
			}
		});
